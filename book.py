...
def log_in(session: HTMLSession, username: str, password: str) -> None:
    session.post("https://www.sit.no/trening",
                 data={"name": username, "pass": password, "form_id": "user_login"}).raise_for_status()
...
def add_booking(session: HTMLSession, token: str, class_id: int) -> None:
    ...
    session.post("https://ibooking.sit.no/webapp/api/Schedule/addBooking", data=data).raise_for_status()
...
if __name__ == '__main__':
    ...
    while current_try <= args.max_tries:
        ...
        try:
            ...
            print("Slot booked!" if success else "Could not find a training slot matching the provided parameters.")
            break
        except Exception:
